...
class U_Next_Story {
	...
	public function enqueue_styles() {
		 ...
		 * @var string $hover_background_color
		 * @var string $hover_text_color
		 * @var string $top_position
		 */

		$rgb_bg_color = $this->hex2rgb( $background_color );
		$rgb_bg_color = implode( ',', $rgb_bg_color );

		$rgb_h_tx_color = $this->hex2rgb( $hover_text_color );
	 ...
	 * @return array
	 */
	public function get_rule_settings( WP_Post $_post = null): array {
		$_post = $_post ?? get_post();
		$settings = wp_cache_get( $_post->ID, $this->_token . '_rule_settings' );
		if ( ! $settings ) {
			...
			$settings  = [
				...
				'background_color'       => get_option( 'u_next_story_background_color', '#ffffff' ),
				'text_color'             => get_option( 'u_next_story_text_color', '#34495e' ),
				'hover_background_color' => get_option( 'u_next_story_hover_background_color', '#34495e' ),
				'hover_text_color'       => get_option( 'u_next_story_hover_text_color', '#ffffff' ),
				'top_position'           => get_option( 'u_next_story_top_position', '50' ),
				'effects_navigation'     => get_option( 'u_next_story_effects_navigation', 'slide' ),
				'scroll_position'        => get_option( 'u_next_story_scroll_position', '0' ),
				'scroll_position_unit'   => get_option( 'u_next_story_scroll_position_unit', 'px' ),
			];
			...
			foreach ( $rules as $rule ) {
				if ( isset( $rule['post_types'] ) && ! empty( $rule['post_types'] ) && is_array( $rule['post_types'] ) && is_singular( $rule['post_types'] ) ) {
					...
					if ( $rule['apply_styles'] === 'on' ) {
						$settings['background_color']       = $rule['background_color'];
						$settings['text_color']             = $rule['text_color'];
						$settings['hover_background_color'] = $rule['hover_background_color'];
						$settings['hover_text_color']       = $rule['hover_text_color'];
						$settings['top_position']           = $rule['top_position'];
						$settings['effects_navigation']     = $rule['effects_navigation'];
						$settings['scroll_position']        = $rule['scroll_position'];
						$settings['scroll_position_unit']   = $rule['scroll_position_unit'];
					}
			...
			wp_cache_set( $_post->ID, $settings, $this->_token . '_rule_settings' );
		}
